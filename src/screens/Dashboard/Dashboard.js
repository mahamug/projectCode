...
import Dashboardflatlist from '../../components/Dashboard/dashboardflatlist';
...
const List = () => {
  ...
  const {cameraPhoto, storeCameraPhoto} = useContext(AppContext);
 ...
 const navigation=useNavigation();
  ...
  return (
    <View style={styles.container}>
      <DrawerNavbar title="Dashboard" navigation={navigation} />
      <View style={Stylesheet.listid}>
        <Image
          source={{uri:Img}}
          style={Stylesheet.idpic}></Image>
        <View style={{marginLeft:20}}>
          <Text
            style={styles.NmaeText}>
            Maheen Awais
          </Text>
          <Text style={styles.membershipText}>Basic </Text>
      ...
      <FlatList
        ...
        data={Catagories}
        renderItem={({item}) => {
          return <Dashboardflatlist item={item}/>
        }}
      />
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  NmaeText:{
    fontSize: 17, color: '#f8e473', fontFamily:"Poppins-Bold"
  },
  membershipText:{
    fontSize: 13, color: 'grey',fontFamily:"Poppins-Regular"
  },
  levelText:{
    fontSize: 13, color: 'grey',fontFamily:"Poppins-Regular"
  },
  Shadow:{
    shadowOffset: {width: 3, height: 6},
        ...
        borderRadius: 15,height: 33, width: 340,marginTop:16,marginLeft:22
  },
  TextInput:{
    color: 'black',padding:3,
  },
  heading:{
    fontSize: 20, fontFamily:"Poppins-SemiBold", color: '#f8e473', margin: 20
  }
})
export default List;
